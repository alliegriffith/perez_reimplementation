...
os.makedirs("perez_models_solo", exist_ok=True)
...
wandb.init(
    ...
    name=f"perez_run_batch{BATCH_SIZE}_solo",
    config={
...
class Critic(nn.Module):
    ...
    def forward(self, transformer_output):
        transformer_output = transformer_output.mean(dim=1) # added to pool over the sequence dimension (tokens) - [B, G, 4096] -> [B, 4096] (took mean hidden state over all tokens)
...
for batch in tqdm(range(NUM_BATCHES)):
    ...
    with amp.autocast("cuda", dtype=torch.bfloat16): # I think this is correct, double check? 
        ...
        kl_divs = (adv_probs * (adv_log_probs - ref_log_probs)).sum(dim=(1, 2)) # sum over all vocab (2) and token per token in batch  -> [B]
        ...
        actor_loss_batches = (batch_log_probs * advantages) # sum across each token -> [B]
